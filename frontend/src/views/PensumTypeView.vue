  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await pensumTypeApi.put(`/${edited.id}`, edited);
      ...
      // Creating record
      try {
        const { data } = await pensumTypeApi.post(null, this.editedItem);
        console.log(data);
        alert.success(data.message);
      } catch (error) {
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await pensumTypeApi.delete(`/${this.editedItem.id}`, {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await pensumTypeApi.get(null, {
