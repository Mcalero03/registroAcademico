...
import prerequisiteApi from "@/services/prerequisiteApi";
...
export default {
  ...
  methods: {
    async initialize() {
      ...
      let requests = [
        this.getDataFromApi(),
        pensumSubjectDetailApi.get(null, {
          params: {
            itemsPerPage: -1,
          },
        }),
      ...
      if (responses) {
        this.details = responses[1].data.data;
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await prerequisiteApi.get(null, {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await prerequisiteApi.put(`/${edited.id}`, edited);
      ...
      // Creating record
      try {
        const { data } = await prerequisiteApi.post(null, this.editedItem);
        alert.success(data.message);
      } catch (error) {
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await prerequisiteApi.delete(
          `/${this.editedItem.id}`,
          {
